# Crack 프로필 관리 보조 2.1.1

크랙의 `설정 → 채팅 → 대화 프로필` 카드에 현재 연결 채팅, 프로필 검색, 사용자 태그/AND 필터, 개인 메모, 메모 글씨 색, 카드 배경색, 프로필 순서 편집 및 일괄 삭제를 추가하는 Chrome/Edge Manifest V3 확장 프로그램입니다.



## 2.1.1 수정

- 기본 태그를 `남`, `여`, `인외` 3개로 정리하고 `기타`는 기본값에서 제외했습니다.
- 2.1.0에서 자동 생성된 미사용 `기타` 태그는 업데이트 시 제거하되, 이미 어떤 프로필에 지정한 `기타` 태그는 사용자 태그로 간주해 보존합니다.

## 2.1.0 추가

- 상단에서 프로필 이름·설명(`information`)·메모를 로컬 부분 검색합니다. 검색 때문에 API를 추가 호출하지 않습니다.
- 기본 태그 `남`, `여`, `인외`를 제공하고 `+`에서 사용자 태그를 추가/삭제할 수 있습니다.
- 각 프로필의 `⋯` 메뉴에 `태그 설정`을 추가해 여러 태그를 동시에 지정할 수 있습니다.
- 여러 태그를 선택하면 OR이 아니라 **AND**로 필터링합니다. 검색어와 태그 조건도 함께 AND로 적용됩니다.
- 카드에 할당된 태그를 작은 chip으로 표시하며, 상단 필터에는 현재 표시 개수를 `표시 / 전체`로 보여줍니다.
- 편집 모드의 `전체 선택`은 필터로 현재 보이는 프로필만 선택합니다. 필터 변경으로 숨겨진 선택 항목은 자동 해제되어 숨은 프로필이 실수로 일괄 삭제되지 않게 했습니다.
- 검색/태그 필터는 저장된 사용자 정렬 순서를 변경하지 않습니다.
- 태그 목록은 `crackProfileTagsV1`, 프로필별 태그는 기존 stable `_id` 기반 설정 객체에 저장합니다.
...
- 메모·메모 색·카드 색·프로필별 태그: `chrome.storage.local`의 기존 `crackProfileConnectionNotesV1`
  - key: 대화 프로필 `_id`
  - 기존 `{ text, updatedAt }`는 그대로 읽음
  - 선택값은 `memoColor`, `cardColor`, `tags`로 추가
  - 필드가 없거나 `default`이면 크랙 원래 테마 색을 사용
- 전체 태그 목록: `crackProfileTagsV1`
  - 기본값: `남`, `여`, `인외`
  - 사용자 추가/삭제 태그도 같은 배열에 저장
